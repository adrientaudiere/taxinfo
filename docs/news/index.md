...
- The GBIF occurrence functions
  ...
  now default to GBIF’s Download API (`method = "download"`), which
  requires GBIF credentials (`GBIF_USER`, `GBIF_PWD`, `GBIF_EMAIL` in
  your `.Renviron`). The previous
  [`rgbif::occ_search()`](https://docs.ropensci.org/rgbif/reference/occ_search.html)
  behaviour is still available with `method = "search"` (no credentials,
  capped at 100,000 records). See
  <https://docs.ropensci.org/rgbif/articles/gbif_credentials.html>.

- [`tax_occur_check()`](https://adrientaudiere.github.io/taxinfo/reference/tax_occur_check.md)
  (and its `*_pq()` wrappers) now report the true worldwide
  georeferenced count in `total_count_in_world` for taxa with no
  occurrence in the search radius, where the previous behaviour returned
  `0`.
...
- [`tax_ecoregion_occur()`](https://adrientaudiere.github.io/taxinfo/reference/tax_ecoregion_occur.md)
  gains a `method` argument (forwarded to
  [`tax_gbif_occur_coords()`](https://adrientaudiere.github.io/taxinfo/reference/tax_gbif_occur_coords.md))
  and keeps the credential-free
  [`rgbif::occ_search()`](https://docs.ropensci.org/rgbif/reference/occ_search.html)
  path as its default, so ecoregion profiling and its wrappers
  ([`tax_ecoregion_occur_pq()`](https://adrientaudiere.github.io/taxinfo/reference/tax_ecoregion_occur_pq.md),
  [`tax_check_ecoregion()`](https://adrientaudiere.github.io/taxinfo/reference/tax_check_ecoregion.md))
  do not require GBIF credentials.

- [`tax_gbif_occur_coords()`](https://adrientaudiere.github.io/taxinfo/reference/tax_gbif_occur_coords.md)
  gains a `method` argument (`"download"`, `"download_sql"`, `"search"`)
  and server-side filter arguments (`country`, `year_gte`, `year_lte`,
  `geometry`). The default `"download"` collapses the former per-taxon
  [`rgbif::occ_search()`](https://docs.ropensci.org/rgbif/reference/occ_search.html)
  loop into a single, citable
  [`rgbif::occ_download()`](https://docs.ropensci.org/rgbif/reference/occ_download.html)
  request and correctly retains infraspecific and higher-rank records.

- [`tax_occur_check()`](https://adrientaudiere.github.io/taxinfo/reference/tax_occur_check.md)
  gains a `method` argument (`"download"`, `"search"`); with
  `method = "download"` it issues a single
  [`rgbif::occ_download()`](https://docs.ropensci.org/rgbif/reference/occ_download.html)
  constrained to the search bounding box instead of
  [`rgbif::occ_search()`](https://docs.ropensci.org/rgbif/reference/occ_search.html).

- [`tax_occur_check_pq()`](https://adrientaudiere.github.io/taxinfo/reference/tax_occur_check_pq.md)
  and
  [`tax_occur_multi_check_pq()`](https://adrientaudiere.github.io/taxinfo/reference/tax_occur_multi_check_pq.md)
  now issue a single GBIF download for all taxa (and, for
  [`tax_occur_multi_check_pq()`](https://adrientaudiere.github.io/taxinfo/reference/tax_occur_multi_check_pq.md),
  all GPS points) when `method = "download"`, instead of one
  [`rgbif::occ_search()`](https://docs.ropensci.org/rgbif/reference/occ_search.html)
  call per taxon per point, and expose `method`, `circle_form`,
  `clean_coord` and `n_occur` arguments.
...
- **Changed default behavior**: The `add_to_phyloseq` parameter now
  defaults to `TRUE` when a phyloseq object is provided, and `FALSE`
  when using the `taxnames` parameter. This makes the workflow more
  intuitive - when working with phyloseq objects, the enriched object is
  returned by default.
...
- All main functions
  ...
  [`tax_occur_check_pq()`](https://adrientaudiere.github.io/taxinfo/reference/tax_occur_check_pq.md),
  [`tax_photos_pq()`](https://adrientaudiere.github.io/taxinfo/reference/tax_photos_pq.md))
  now support the `taxnames` parameter, allowing users to query
  information for specific taxonomic names without a phyloseq object.

- Added comprehensive tests for `taxnames` parameter usage across all
  functions.
...
- Add functions
  ...
  to retrieve spore size information from MycoDB.

- Add params `year_col` and `authorship_col` to
  [`gna_verifier_pq()`](https://adrientaudiere.github.io/taxinfo/reference/gna_verifier_pq.md)
  to output year of publication and authorship information for each
  taxa.

- Add function
  [`intra_taxnames_dist()`](https://adrientaudiere.github.io/taxinfo/reference/intra_taxnames_dist.md)
  to compute pairwise DNA distances among taxa with the same taxonomic
  names.

- Add function
  [`cluster_sbc()`](https://adrientaudiere.github.io/taxinfo/reference/cluster_sbc.md)
  to (post)cluster taxa into SBC (Species bound cluster) defined as
  “clusters that include all and only ESVs assigned to one species, the
  sequence similarity threshold can vary between these clusters” by
  Riley *et al.* 2025 (<https://doi.org/10.1186/s12915-025-02284-x>).
  Also add a new vignette to illustrate the use of
  [`cluster_sbc()`](https://adrientaudiere.github.io/taxinfo/reference/cluster_sbc.md).

- Print information when using
  [`tax_info_pq()`](https://adrientaudiere.github.io/taxinfo/reference/tax_info_pq.md)
  with `add_to_phyloseq = TRUE` to inform users that the phyloseq object
  is being updated.

- Add an example in
  [`tax_info_pq()`](https://adrientaudiere.github.io/taxinfo/reference/tax_info_pq.md)
  manual with the EPPO database to determine if pest species regulated
  in France are found in the example phyloseq object.

- Change the result column `genus` into `genusEpithet` from the
  [`gna_verifier_pq()`](https://adrientaudiere.github.io/taxinfo/reference/gna_verifier_pq.md)
...
- Fixed issue in functions
  ...
  and
  [`range_bioreg_pq()`](https://adrientaudiere.github.io/taxinfo/reference/range_bioreg_pq.md)
  due to the loss of the column verbatim_index in
  [`rgbif::name_backbone_checklist()`](https://docs.ropensci.org/rgbif/reference/name_backbone_checklist.html)
  (commit
  [c74602b](https://github.com/ropensci/rgbif/commit/c74602b1c50e9c5d1a1fd6251dcad23595a94345)).

### Documentation

- Updated documentation for all functions to clarify the new default
  behavior of `add_to_phyloseq`.
- Added examples showing both phyloseq and taxnames usage patterns.
- Updated vignettes to demonstrate the dual-input capability (phyloseq
  objects vs. taxonomic name vectors).
- Updated README to highlight the flexible input options.

## taxinfo 0.1.1

- Add `list_keywords` and `n_citation` columns in the return of
  [`tax_oa_pq()`](https://adrientaudiere.github.io/taxinfo/reference/tax_oa_pq.md).
