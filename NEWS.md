...
* `fungal_traits_guilds()` now compares the FungalTraits and FUNGuild trophic modes taxon by taxon in `cons_trophicMode` and `cons_trophicMode_agreement`: a taxon whose FUNGuild mode contains its own FungalTraits mode (e.g. `Saprotroph` in `Pathotroph-Saprotroph`) gets that mode and `"Agree"` instead of `"Conflicting"` / `"Disagree"`, and FUNGuild values are trimmed. Consensus columns computed with earlier versions should be recomputed.
* `gbif_key_to_col()` converts legacy GBIF Backbone numeric taxon keys to Catalogue of Life Extended Release (COL XR) alpha-numeric keys (a character vector aligned with the input, `NA` when unmatched, or a new `col_key` column when given a data frame with a `usageKey` column; requires rgbif >= 3.9.0), preparing taxinfo for the rgbif 3.9.0 breaking change. All GBIF name matching now runs through a checklist-pinned layer (`gbif_backbone_checklist()`, `gbif_backbone()`, `gbif_name_usage()`) whose checklist is controlled by the new `taxinfo.gbif_checklist` option (`"colxr"` or `"backbone"`; when unset, taxinfo follows the installed rgbif's default checklist and sends no `checklistKey`), so resolved keys no longer silently change across rgbif versions; `tax_iucn_code_pq()` and `tax_photos_pq()` gain a `checklist` argument (default `"backbone"`, required by the GBIF-Backbone-only `rgbif::name_usage()` API), and GBIF record attribution compares keys in a type-agnostic way. GBIF occurrence queries (`rgbif::occ_search()`, occurrence counts and predicate downloads) now send the `checklistKey` of the resolved keys when it differs from the installed rgbif's default, so `taxinfo.gbif_checklist = "colxr"` no longer silently returns zero occurrences with rgbif < 3.9.0 (predicate downloads of COL XR keys require rgbif >= 3.9.0 and abort otherwise); `method = "download_sql"` re-matches COL XR taxa in the GBIF Backbone, the only checklist of the SQL `taxonkey` / `specieskey` columns.
...
* `tax_bactotraits_pq()` augments the `tax_table` of a bacterial phyloseq object with the BactoTraits functional-trait database (Laderriere et al. 2026), joining its species- and genus-level CSV files (downloaded once from <https://doi.org/10.24396/ORDAR-182> and cached, or passed via `file_name_species` / `file_name_genus`) with a species-to-phylum rank fallback whose coarser-level profiles are strain-weighted means; the `Species` rank may hold binomials, bare epithets or the genus glued to the epithet (e.g. `"Sulfolobusacidocaldarius"` in `GlobalPatterns`). Binary traits (e.g. `motility_yes` / `motility_no`) collapse to one `TRUE` / `FALSE` / probability column per trait (`merge_modalities`), while `raw_cols = TRUE` keeps the raw affinity columns and multi-modality traits always keep theirs.
* `tax_check_ecoregion()`, `tax_ecoregion_occur()` and `tax_ecoregion_occur_pq()` gain an opt-in `saturation` early-stop (`saturation = TRUE`, default `FALSE`; only used with `method = "search"`): GBIF occurrences are fetched in batches of `batch_size` and fetching stops as soon as the taxon's ecoregion set stops growing for `patience` consecutive batches (with a `min_points` floor and the `n_occur` cap), so an ecoregion profile needs only as many GBIF points as it takes to stabilise; the per-taxon fetch statistics are the `"saturation"` attribute of the long ecoregion tibble (`attr(result, "saturation")` for `tax_ecoregion_occur()` and `tax_ecoregion_occur_pq(add_to_phyloseq = FALSE)`, `attr(result$taxon_ecoregions, "saturation")` for `tax_check_ecoregion()`).
* `tax_funendo_pq()` summarises the FunEndo global fungal endophyte repository (Moghaddam 2026, <https://doi.org/10.65390/fdiv.2026.136016>) per taxon: record, reference and accession counts (exact duplicate records removed; references identified by their normalised DOI, or by their reference text when the DOI field holds none, e.g. `"Unpublished"`) plus the collapsed organs, study types, reference types, countries and host genera, matching binomials first with a genus fallback (`match_level`, `max_levels`, `collapse`). The repository is bundled as `inst/extdata/funendo_v0.1.csv.gz`, with `funendo_mini.csv` and `funendo_field_description.csv` for quick examples.
* `tax_gbif_alt()` attributes downloaded records to the queried taxa by `taxonKey` or `speciesKey`, falling back to the taxon name, so infraspecific records are kept and COL XR queries, whose records carry GBIF Backbone keys, are no longer emptied.
* `tax_gbif_alt()`, `tax_gbif_occur_pq()`, `tax_occur_check_pq()`, `tax_occur_multi_check_pq()` and `tax_photos_pq()` no longer error when none of the queried names match GBIF: they warn, send no GBIF request (in particular no occurrence download without a taxon filter), and return their usual output with no rows or `NA` values. `tax_photos_pq()` also no longer errors when none of the matched taxa has a photo.
* `tax_gbif_occur_coords()` no longer errors when none of the queried names match GBIF: it returns an empty result whose `missing_taxa` attribute lists them, so `tax_ecoregion_occur()` and its wrappers keep these taxa as rows with `n_occur = 0`, as documented.
