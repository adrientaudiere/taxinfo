...
As most of `taxinfo`'s utilities, these functions need a phyloseq object as input with cleaned taxonomic names (or a list of individual taxa names). The algorithm used to assign the taxonomic names must be stringent enough to avoid false positives. Moreover, we recommend using [gna_verifier_pq()] to clean taxonomic names by disambiguating synonyms and correcting misspellings. 
