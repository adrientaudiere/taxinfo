...
```{r, fig.alt="Scatter plot showing Wikipedia page views (log scale) versus number of Wikipedia language versions for taxa across four languages (en, fr, de, es). Points are sized by mean page length and labeled with taxonomic names in italic font."}
...
ggplot(interaction_summary, aes(
  ...
  geom_violin() +
  geom_jitter(col = "grey40", alpha = 0.2) +
  labs(
...
```{r echo=FALSE, fig.alt="Horizontal bar chart showing fungal lifestyle distribution. Bars are colored by primary lifestyle category (e.g., saprotroph, pathogen) and ordered by occurrence (number of samples). Text labels show total sequence abundance for each taxon. Taxon names are displayed in italic font.", out.width="100%"}
...
```{r, fig.alt="Horizontal bar chart showing frequency of taxa with French common names from TAXREF database. Bars are colored by Order and show number of sample presences. Taxa known in France (present in TAXREF) are shown in bold italic while others are in regular italic. French vernacular names are shown as text labels."}
...
```{r, fig.alt="Scatter plot showing integrated taxonomic information with GBIF occurrences (x-axis, log scale) versus Wikipedia page views (y-axis, log scale). Points are colored by Order, shaped by presence in France (TAXREF), and sized by number of publications (log scale). Taxonomic names are labeled in italic font."}
